...
import fr.benjamin.cap_entreprise.utils.JspUtils;
    ...
    public File writeExcel() {
        ...
        try (OutputStream os = new FileOutputStream(serverFile)) {
            ...
            for (int i = 1; i <= reviews.size(); i++) {
                ...
                ws.value(i, 0, DateUtils.convertLocalDateTimeToFormat(review.getCreatedAt(), "dd/MM/YYYY"));
                ws.value(i, 1, review.getGame().getName());
                ws.value(i, 2, review.getPlayer().getUsername());
                ws.value(i, 3, jspUtils.getStringRating(review.getRating()));
                ws.value(i, 4, isModerated ? "À modérer" : "Modéré");
                ws.value(i, 5, isModerated ? "" : DateUtils.convertLocalDateTimeToFormat(review.getModeratedAt(), "dd/MM/YYYY"));
            }
            wb.finish();
            return serverFile;
        } catch (IOException e) {
